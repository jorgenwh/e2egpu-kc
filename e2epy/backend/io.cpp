...
int FastaReader::read_raw_chunk(char **chunk, const int chunk_size)
{
  *chunk= new char[chunk_size];
  file_obj_m->read(*chunk, chunk_size);
  int bytes = file_obj_m->gcount();
  bytes_read_m += bytes;
  done_m = (bytes_read_m >= file_length_m);
  return bytes;
}
